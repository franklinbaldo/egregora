...
class VectorStore:
    ...
    def _validate_and_normalize_mode(self, mode: str) -> str:
        ...
        # Fallback to exact search if VSS is not available
        if mode_normalized == "ann" and self._vss_function is None:
            logger.info("VSS extension unavailable; falling back to exact search")
            return "exact"
    ...
    def _handle_ann_failure(
        ...
        # Check for various VSS failure modes to trigger fallback
        ...
        if not self._vss_function or "does not support the supplied arguments" in error_msg:
            ...
            should_fallback = True
        elif "failed to load" in error_msg:
            should_fallback = True

        if should_fallback:
            logger.info("Falling back to exact search (VSS unavailable or failed: %s)", last_error)
            try:
                query = self._build_exact_query() + where_clause + order_clause
                return self._execute_search_query(query, params, min_similarity_threshold)
            except Exception:
