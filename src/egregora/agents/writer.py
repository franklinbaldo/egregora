...
def build_rag_context_for_prompt(  # noqa: PLR0913
    ...
    *,
    ...
    if cache and not cache.should_refresh(CacheTier.RAG):
        ...
        rag_cache_key = (
            f"{query_hash}:{index_version}:{embedding_model}:{top_k}:"
            f"{retrieval_mode}:{retrieval_nprobe}:{retrieval_overfetch}"
        )
...
def write_posts_for_window(  # noqa: PLR0912, C901 - Complexity from caching logic added by Jules PR #890
    table: Table,
    window_start: datetime,
    window_end: datetime,
    ctx: PipelineContext,
    ...
    try:
        ...
        if deps.prompts_dir:
            ...
            if tpl_path.exists():
                template_content = tpl_path.read_text()
    except OSError:  # File access errors
