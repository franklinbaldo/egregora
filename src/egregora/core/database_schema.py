...
def ensure_message_schema(
    ...
    *,
    ...
    """Return ``table`` cast to the canonical :data:`CONVERSATION_SCHEMA`.
    ...
    strictly enforces CONVERSATION_SCHEMA by:
    - Adding missing columns with nulls
    - Casting existing columns to correct types
    - Dropping any extra columns not in CONVERSATION_SCHEMA
    - Normalizing timezone information
    """

    # Convert Ibis schema to dict for easier manipulation
    target_schema = dict(CONVERSATION_SCHEMA)
    ...
    # Drop any extra columns not in CONVERSATION_SCHEMA
