...
def apply_command_to_profile(
    ...
    elif cmd_type in {"set", "unset"} and target == "avatar":
        # Avatar commands are handled separately by the pipeline
        # This is a placeholder - actual processing happens in avatar.py
        # Just log that we received the command
        logger.info(
            f"Avatar command received for {author_uuid}: {cmd_type} avatar "
            f"(processing happens in pipeline)"
        )
        # Don't modify the profile here - avatar.py will update it after moderation
        return str(profile_path)
