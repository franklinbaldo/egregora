...
EGREGORA_TEMPLATES_DIR = Path(__file__).resolve().parent.parent / "templates"
...
def _create_default_mkdocs(mkdocs_path: Path, site_root: Path) -> Path:
    ...
    # Load from parent templates directory to access both site/ and .egregora/
    env = Environment(
        loader=FileSystemLoader(str(EGREGORA_TEMPLATES_DIR)),
    ...
    mkdocs_template = env.get_template("site/mkdocs.yml.jinja2")
...
def _create_site_structure(
    ...
    if not readme_path.exists():
        template = env.get_template("site/README.md.jinja2")
    ...
    if not gitignore_path.exists():
        template = env.get_template("site/.gitignore.jinja2")
        content = template.render(**context)
    ...
    if blog_dir != "." and not homepage_path.exists():
        template = env.get_template("site/docs/index.md.jinja2")
        content = template.render(**context)
    ...
    if not about_path.exists():
        template = env.get_template("site/docs/about.md.jinja2")
        content = template.render(**context)
    ...
    if not blog_index_path.exists():
        template = env.get_template("site/docs/posts/index.md.jinja2")
        content = template.render(**context)
    ...
    if not profiles_index_path.exists():
        template = env.get_template("site/docs/profiles/index.md.jinja2")
        content = template.render(**context)
    ...
    if not media_index_path.exists():
        template = env.get_template("site/docs/media/index.md.jinja2")
        content = template.render(**context)
...
def _render_egregora_config(site_root: Path, env: Environment, context: dict[str, Any]) -> None:
    ...
    """
    egregora_template_dir = EGREGORA_TEMPLATES_DIR / ".egregora"
    ...
    for template_path in egregora_template_dir.rglob("*"):
        if template_path.is_file():
            ...
            # Get template relative to templates root (for Jinja loader)
            template_rel = template_path.relative_to(EGREGORA_TEMPLATES_DIR)
