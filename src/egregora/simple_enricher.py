...
def get_mimetype_subfolder(media_path: Path, media_type: str) -> str:
    ...
    if media_type == "image" or media_path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']:
        return "images"
    elif media_type == "video" or media_path.suffix.lower() in ['.mp4', '.webm', '.ogg', '.mov']:
        return "videos"
    elif media_type == "audio" or media_path.suffix.lower() in ['.mp3', '.wav', '.ogg', '.m4a', '.opus']:
        return "audio"
    elif media_path.suffix.lower() in ['.pdf', '.docx', '.doc', '.txt', '.rtf', '.odt', '.pptx', '.ppt', '.xlsx', '.xls']:
        return "documents"
    else:
        return "files"
...
def save_simple_enrichment(
    ...
    if url:
        ...
        lines.append("")
        lines.append(f'<iframe src="{url}" width="100%" height="500" frameborder="0" loading="lazy"></iframe>')
        lines.append("")
    ...
    if media_path and media_path.exists():
        ...
        try:
            ...
            if media_type == "image" or (media_path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']):
                ...
                lines.append(f'<video controls width="100%">')
                lines.append(f'  <source src="{media_filename}" type="video/{media_path.suffix[1:]}">')
                lines.append(f'  Seu navegador não suporta o elemento de vídeo.')
                lines.append(f'</video>')
            elif media_type == "audio" or (media_path.suffix.lower() in ['.mp3', '.wav', '.ogg', '.m4a']):
                lines.append(f'<audio controls>')
                lines.append(f'  <source src="{media_filename}" type="audio/{media_path.suffix[1:]}">')
                lines.append(f'  Seu navegador não suporta o elemento de áudio.')
...
def save_media_enrichment(
    ...
    if media_path.exists():
        ...
        if media_type == "image" or (media_path.suffix.lower() in ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.svg']):
            ...
            lines.append(f'<embed src="{media_filename}" type="application/pdf" width="100%" height="600px" />')
            lines.append(f'<p><a href="{media_filename}">📄 Abrir PDF em nova aba</a></p>')
        elif media_path.suffix.lower() in ['.docx', '.doc', '.txt', '.rtf', '.odt', '.pptx', '.ppt', '.xlsx', '.xls']:
            ...
            lines.append(f'<p><em>Clique no link acima para baixar e visualizar o documento.</em></p>')
        else:
            # Generic file download link
            lines.append(f"[📁 Download: {media_filename}]({media_filename})")
