...
@retry(
    ...
    retry=retry_if_exception_type((httpx.HTTPError, RateLimitError, EmbeddingAPIError)),
    reraise=True,
)
def _call_with_retries(func: Any) -> Any:
    """Retry wrapper for HTTP calls with tenacity.

    Wraps HTTP library exceptions in domain-specific exceptions to hide
    ...
    try:
        ...
        raise EmbeddingAPIError(msg, original_error=e) from e
...
def embed_text(
    ...
    *,
    ...
    """Embed a single text using the Google Generative AI API.
