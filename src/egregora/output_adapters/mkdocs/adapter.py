...
class MkDocsAdapter(OutputAdapter):
    ...
    **ISP-COMPLIANT** (2025-11-22): This adapter implements both:
    ...
    def read_document_by_url(self, url_path: str) -> Document | None:  # noqa: C901
        ...
        # Try to detect document type from path structure and find the file
        # Posts: posts/YYYY-MM-DD-slug/
        if url_path.startswith("posts/"):
            slug = url_path[len("posts/") :]
            # Find matching file in posts directory
        ...
        elif url_path.startswith("profiles/"):
            uuid = url_path[len("profiles/") :]
            path = self.profiles_dir / f"{uuid}.md"
            doc_type = DocumentType.PROFILE

        # Journal: journal/identifier/
        elif url_path.startswith("journal/"):
            identifier = url_path[len("journal/") :]
            safe_identifier = identifier.replace("/", "-")
            path = self.journal_dir / f"{safe_identifier}.md"
            doc_type = DocumentType.JOURNAL

        # URLs: urls/hash/
        elif url_path.startswith("urls/"):
            hash_id = url_path[len("urls/") :]
            path = self.urls_dir / f"{hash_id}.md"
            doc_type = DocumentType.ENRICHMENT_URL

        # Media enrichments: media/hash/
        elif url_path.startswith("media/") and url_path.count("/") == 1:
            hash_id = url_path[len("media/") :]
            path = self.media_dir / f"{hash_id}.md"
            doc_type = DocumentType.ENRICHMENT_MEDIA

        # Media files: media/filename
        elif url_path.startswith("media/"):
            filename = url_path[len("media/") :]
            path = self.media_dir / filename
            doc_type = DocumentType.MEDIA

        else:
            logger.debug("Unknown URL path format: %s", url_path)
            return None

        # Check if file exists
        if not path.exists():
            logger.debug("Document not found at path: %s", path)
            return None
