...
class MkDocsProfileStorage:
    """Filesystem-based profile storage with MkDocs blog integration."""

    def __init__(self, site_root: Path) -> None:
        self.site_root = site_root
        self.profiles_dir = site_root / PROFILES_DIR_NAME
        self.profiles_dir.mkdir(parents=True, exist_ok=True)

    def write(self, author_uuid: str, content: str) -> str:
        """Persist a profile and refresh ``.authors.yml`` for the blog plugin."""
        from egregora.agents.shared.author_profiles import write_profile as write_profile_content
    ...
    def _update_authors_yml(self, author_uuid: str, profile_path: Path) -> None:
        """Update ``.authors.yml`` with metadata extracted from ``profile_path``."""
        ...
        try:
            ...
            logger.warning("Failed to read profile for .authors.yml update: %s", exc)
            return
        ...
        if authors_yml_path.exists():
            try:
                authors = yaml.safe_load(authors_yml_path.read_text(encoding="utf-8")) or {}
        ...
        try:
            ...
            logger.warning("Failed to write .authors.yml: %s", exc)

    @staticmethod
    def _extract_profile_metadata(content: str) -> dict[str, Any]:
        """Extract YAML front matter from ``content`` when present."""
        ...
        try:
            ...
            logger.debug("Failed to parse profile frontmatter", exc_info=True)
        return {}
...
class MkDocsJournalStorage:
    ...
    def _sanitize_label(label: str) -> str:
        """Convert a window label into a filename-safe string."""
...
class MkDocsEnrichmentStorage:
    ...
    def __init__(self, site_root: Path) -> None:
        self.site_root = site_root
        self.urls_dir = site_root / MEDIA_DIR_NAME / "urls"
        self.urls_dir.mkdir(parents=True, exist_ok=True)

    def write_url(self, url: str, content: str, metadata: dict[str, Any] | None = None) -> str:
        """Persist a URL enrichment using deterministic, human-friendly filenames."""
        metadata = metadata or {}
        url_prefix = slugify(url, max_len=40)
        url_hash = uuid_lib.uuid5(uuid_lib.NAMESPACE_URL, url).hex[:8]
    ...
    def write_media(self, filename: str, content: str) -> str:
        """Store a media enrichment alongside the media asset."""
        media_path = safe_path_join(self.site_root, filename)
