...
This module centralizes MkDocs site path discovery so callers across the
codebase share a single implementation. It was extracted from
``egregora.config.site`` to avoid diverging logic between configuration and
output layers.
"""
...
def resolve_site_paths(start: Annotated[Path, "The starting directory for path resolution"]) -> SitePaths:
    ...
    blog_dir = _extract_blog_dir(_config) or DEFAULT_BLOG_DIR
