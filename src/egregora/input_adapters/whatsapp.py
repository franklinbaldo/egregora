...
def extract_commands(messages: Table) -> list[dict]:
    ...
    commands_table = command_cases.filter(command_cases.command_name.notnull()).select(  # noqa: PD004 - Ibis uses notnull(), not notna()
...
def build_message_attrs(
    ...
    has_metadata = ibis.coalesce(original_line, tagged_line, message_date).notnull()  # noqa: PD004 - Ibis uses notnull(), not notna()
