...
def _run_offline_demo(output_dir: Path) -> None:
    ...
    # Define placeholder context for the template
    placeholder_post = {
        "title": "Welcome to Egregora!",
        "summary": "This is a placeholder post to demonstrate the layout. Generate your own content with 'egregora write'.",
        "date": "2025-01-01",
        "reading_time": 1,
        "url": "posts/2025/01/01/welcome/",
        "authors": [{"name": "Egregora", "avatar": "assets/images/favicon.svg"}],
        "tags": ["Demo", "Welcome"],
        "banner": None,
    }

    # 1. Scaffold the site with context overrides
    scaffolder = MkDocsSiteScaffolder()
    scaffolder.scaffold_site(
        output_dir,
        site_name="Egregora Demo (Offline)",
        context_overrides={"posts": [placeholder_post]},
    )

    # 2. Create the actual placeholder post file so the link works
