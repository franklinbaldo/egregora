"""Commands for running the content generation pipeline."""
...
from egregora.cli._app import app, console, logger
...
def _ensure_mkdocs_scaffold(output_dir: Path) -> None:
    """Ensure the MkDocs scaffold exists, prompting the user when needed."""
...
def _validate_and_run_process(config: ProcessConfig, source: str = "whatsapp") -> None:
    ...
    if config.timezone:
        try:
            ...
            console.print(f"[red]Invalid timezone '{config.timezone}': {exc}[/red]")
