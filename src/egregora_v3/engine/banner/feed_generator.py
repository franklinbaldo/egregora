...
class FeedBannerGenerator:
    ...
    def _generate_sequential(self, entries: list[Entry]) -> list[BannerGenerationResult]:
        ...
        for entry in entries:
            banner_input = self._entry_to_banner_input(entry)
            result = self._generate_with_provider(entry, banner_input)
            results.append(result)
        return results

    def _entry_to_banner_input(self, entry: Entry) -> BannerInput:
        """Convert an Entry to a BannerInput."""
        return BannerInput(
            post_title=entry.title,
            post_summary=entry.summary or "",
            slug=entry.internal_metadata.get("slug")
            if entry.internal_metadata
            else None,
            language=entry.internal_metadata.get("language", "pt-BR")
            if entry.internal_metadata
            else "pt-BR",
        )

    def _generate_with_provider(
        self, entry: Entry, banner_input: BannerInput
        ...
        if result.has_image and result.image_bytes:
            ...
            return BannerGenerationResult(entry, document=document)
