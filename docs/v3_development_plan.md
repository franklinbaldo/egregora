...
*   **1.1 Types:** Define `Document`, `Message`, `DocumentType`. Implement content-addressed ID generation.
*   **1.2 Config:** Implement `EgregoraConfig` using Pydantic V2 with strict path resolution.
*   **1.3 Ports:** Define Protocols (`InputAdapter`, `DocumentRepository`, `VectorStore`, `LLMModel`, `UrlConvention`, `OutputSink`).

### Phase 2: Data Infrastructure
*   **2.1 Adapter:** Implement `WhatsAppAdapter` to parse export files into `Message` streams.
*   **2.2 Repository:** Implement `DuckDBRepository` using Ibis for synchronous CRUD operations on `Document` and `Message`.
...
*   **3.3 Agents:**
    *   `EnricherAgent`: Enriches messages/media.
    *   `WriterAgent`: Generates posts from message windows.

### Phase 4: Pipeline Orchestration
*   **4.1 Windowing:** Implement `WindowingEngine` to split message streams into overlapping windows based on time or count.
