...
*   **2.2 Output:** Implement `MkDocsAdapter` (OutputSink) and `StandardUrlConvention` for persisting documents to disk.
*   **2.3 Privacy Adapter:** Port/Re-implement `anonymize.py` logic to ensure privacy compliance before ingestion.
*   **2.4 Input Adapter:** Implement `WhatsAppAdapter` leveraging the Privacy Adapter to parse export files.
...
*   **Unit Tests:** Strict isolation for Agents and Core logic.
    *   **Property-Based Testing:** Use `hypothesis` for invariant checking on `Document` and `FeedItem`.
