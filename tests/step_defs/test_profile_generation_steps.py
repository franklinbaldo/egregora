...
def chat_history_table(context, datatable):
    ...
    for row in data:
        messages.append(
            {"author_uuid": row["author_uuid"], "author_name": row["author_name"], "text": row["text"]}
        )
...
def chat_history_count(context, count, author):
    messages = [
        {"author_uuid": f"{author}-uuid", "author_name": author, "text": f"Message {i}"} for i in range(count)
    ]
    context["messages"] = messages
