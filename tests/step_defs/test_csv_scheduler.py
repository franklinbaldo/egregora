...
# Mock the jules module to avoid import errors
sys.modules["jules"] = MagicMock()
sys.modules["jules.scheduler"] = MagicMock()
sys.modules["jules.scheduler.schedule"] = MagicMock()
sys.modules["jules.scheduler.engine"] = MagicMock()
...
def schedule_has__rows(count, mock_schedule_path):
    with mock_schedule_path.open(newline="") as f:
        reader = csv.DictReader(f)
        rows = list(reader)
