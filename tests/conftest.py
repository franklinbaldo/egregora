...
@pytest.fixture(scope="session")
def writer_test_model_factory() -> type[TestModel]:
    """Deterministic ``TestModel`` matching the writer agent contract."""

    class CliWriterTestModel(TestModel):
        def __init__(self, *, window_id: str):
            self._window_id = window_id
            super().__init__(
                call_tools=["write_post_tool", "write_profile_tool"],
                custom_output_args={
                    "summary": "Deterministic writer summary",
                    "notes": "Structured output from CliWriterTestModel",
                },
                seed=2025,
            )
        ...
        def gen_tool_args(self, tool_def):  # type: ignore[override]
            if tool_def.name == "write_post_tool":
                return {
                    "metadata": {
                        ...
                        "authors": ["ca71a986"],
                        "summary": "Synthetic summary generated for CLI validation.",
                    },
                    "content": (
                        "# Deterministic Post\n\n"
                        "This content is emitted by CliWriterTestModel to keep the CLI test stable."
                    ),
                }
            ...
            if tool_def.name == "write_profile_tool":
                return {
                    ...
                    "content": "Profile content generated by CliWriterTestModel for deterministic runs.",
                }

            return super().gen_tool_args(tool_def)

    return CliWriterTestModel


@pytest.fixture(scope="session")
def deterministic_embedding_vector():
    """Generate deterministic embedding vectors matching Gemini output shape."""

    def _generate(text: str, *, dimensionality: int = 768) -> list[float]:
...
@pytest.fixture
def writer_test_model(monkeypatch, writer_test_model_factory):
    """Patch the writer agent to run with a deterministic ``TestModel``."""

    from egregora.agents.writer import agent as writer_agent

    original = writer_agent.write_posts_with_pydantic_agent

    def _patched(*, prompt, config, context, **kwargs):
        model = writer_test_model_factory(window_id=context.start_time.date().isoformat())
        return original(prompt=prompt, config=config, context=context, test_model=model, **kwargs)

    monkeypatch.setattr(writer_agent, "write_posts_with_pydantic_agent", _patched)
    return writer_test_model_factory


@pytest.fixture
def embedder_stub(monkeypatch, deterministic_embedding_vector):
    """Stub embedding helpers with deterministic vectors for offline tests."""

    from egregora.agents.shared.rag import embedder as embedder_module
    ...
    def _embed_text(
        ...
        *,
        ...
        return deterministic_embedding_vector(text)

    def _embed_texts_in_batch(
        texts,
        *,
        model: str,
        task_type: str | None = None,
        ...
        return [deterministic_embedding_vector(text) for text in texts]

    def _embed_chunks(chunks, *, model: str, task_type: str = "RETRIEVAL_DOCUMENT"):
        return [deterministic_embedding_vector(chunk) for chunk in chunks]

    monkeypatch.setattr(embedder_module, "embed_text", _embed_text)
    monkeypatch.setattr(embedder_module, "embed_texts_in_batch", _embed_texts_in_batch)
    monkeypatch.setattr(embedder_module, "embed_chunks", _embed_chunks)

    return _embed_texts_in_batch
