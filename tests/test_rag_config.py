...
def test_rag_config_accepts_supported_fields(tmp_path: Path) -> None:
    ...
    assert config.retrieval.min_similarity == MIN_SIMILARITY
    assert config.query.keyword_stop_words == ("alpha", "beta")
    assert config.embedding.cache_dir == tmp_path / "cache"
    assert config.embedding.embedding_export_path == tmp_path / "embeddings.parquet"
    assert config.vector_store.persist_dir == tmp_path / "vector_store"
...
def test_rag_config_validates_overlap_bounds() -> None:
    with pytest.raises(ValidationError):
        RAGConfig(chunking=dict(chunk_size=256, chunk_overlap=512))
