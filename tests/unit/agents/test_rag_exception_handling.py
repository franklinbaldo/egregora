...
def mock_pipeline_context():
    ...
    # Ensure with_output_sink returns self to chain calls
    ctx.with_output_sink.return_value = ctx
    ctx.with_adapter.return_value = ctx
    return ctx
...
def test_prepare_pipeline_data_handles_rag_connection_error(
    ...
    with patch("egregora.orchestration.pipelines.etl.preparation.index_documents") as mock_index:
        ...
        with patch("egregora.orchestration.pipelines.etl.preparation.PipelineFactory") as mock_factory:
            # Setup factory to return our mock context's output sink
