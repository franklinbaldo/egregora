...
def test_elo_store_append_only_updates(memory_storage):
    ...
    # Be flexible with datetime precision or timezone info if backend returns native types
    assert fetched_latest.last_updated.timestamp() == pytest.approx(t2.timestamp())
