...
from egregora.knowledge.taxonomy import generate_semantic_taxonomy
...
def test_generate_semantic_taxonomy_insufficient_docs(mock_output_sink, mock_config):
    ...
    with patch("egregora.knowledge.taxonomy.get_backend") as mock_get_backend:
        backend = MagicMock()
        rng = np.random.default_rng(seed=42)
...
def test_generate_semantic_taxonomy_success(mock_output_sink, mock_config):
    ...
    with (
        patch("egregora.knowledge.taxonomy.get_backend") as mock_get_backend,
        patch("egregora.knowledge.taxonomy.create_global_taxonomy_agent") as mock_create_agent,
    ):
        # Setup Backend
        backend = MagicMock()
...
def test_generate_semantic_taxonomy_batching(mock_output_sink, mock_config):
    ...
    with (
        patch("egregora.knowledge.taxonomy.get_backend") as mock_get_backend,
        patch("egregora.knowledge.taxonomy.create_global_taxonomy_agent") as mock_create_agent,
        patch("egregora.knowledge.taxonomy.MAX_PROMPT_CHARS", 100),
    ):  # FORCE tiny limit
        # Setup Backend
        backend = MagicMock()
        real_docs = list(mock_output_sink.documents())
