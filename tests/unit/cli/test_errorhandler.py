...
from egregora.config.exceptions import ApiKeyNotFoundError, InvalidConfigurationValueError
from egregora.input_adapters.exceptions import UnknownAdapterError
from egregora.orchestration.exceptions import CommandAnnouncementError, ApiKeyInvalidError
    ...
    with patch("egregora.cli.errorhandler.console.print") as mock_print:
        with pytest.raises(typer.Exit) as excinfo:
            with handle_cli_errors(debug=False):
                raise ApiKeyNotFoundError("TEST_ENV_VAR")
...
def test_handle_cli_errors_api_key_invalid():
    ...
    with patch("egregora.cli.errorhandler.console.print") as mock_print:
        with pytest.raises(typer.Exit) as excinfo:
            with handle_cli_errors(debug=False):
                raise ApiKeyInvalidError("Invalid Key")
...
def test_handle_cli_errors_unknown_adapter():
    ...
    with patch("egregora.cli.errorhandler.console.print") as mock_print:
        with pytest.raises(typer.Exit) as excinfo:
            with handle_cli_errors(debug=False):
                raise UnknownAdapterError("bad_source")
