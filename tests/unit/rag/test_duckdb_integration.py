...
def test_search_to_table_basic(mock_rag_response):
    ...
    mock_backend.query.return_value = mock_rag_response

    request = RAGQueryRequest(text="test query", top_k=5)
    table = search_to_table(mock_backend, request)
...
def test_search_to_table_empty_results():
    ...
    mock_backend = Mock(spec=RAGBackend)
    mock_backend.query.return_value = empty_response
...
def test_search_with_filters_min_score(mock_rag_response):
    ...
    result_table = search_with_filters(mock_backend, "test query", min_score=0.8, top_k=10)
...
def test_search_with_filters_document_type(mock_rag_response):
    ...
    result_table = search_with_filters(mock_backend, "test query", min_score=0.0, document_types=["POST"], top_k=10)
    result = result_table.execute()
