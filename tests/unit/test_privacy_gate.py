...
class TestRequirePrivacyPassDecorator:
    ...
    def test_decorator_fails_without_privacy_pass(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="protected_function requires PrivacyPass capability"
        ):
    ...
    def test_decorator_fails_with_forged_token(self):
        ...
        with pytest.raises(
            RuntimeError,
            match="received invalid privacy_pass.*Expected PrivacyPass instance, got str"
        ):
        ...
        with pytest.raises(
            RuntimeError,
            match="received invalid privacy_pass.*Expected PrivacyPass instance, got dict"
        ):
            protected_function("test data", privacy_pass={"fake": "token"})  # type: ignore[arg-type]
