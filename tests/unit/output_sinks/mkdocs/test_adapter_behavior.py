...
from egregora.output_sinks.exceptions import FileWriteError, ProfileMetadataError, DocumentNotFoundError, AdapterNotInitializedError
...
def adapter(tmp_path):
    ...
    adapter.initialize(site_root=site_root, url_context=UrlContext(base_url="https://example.com", site_prefix=""))
    return adapter
...
def test_persist_journal_behavior(adapter):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/journal/my-journal"):
        adapter.persist(doc)
...
def test_persist_profile_behavior(adapter):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value=f"https://example.com/authors/{uuid}/jane-doe"):
        adapter.persist(doc)
...
def test_persist_enrichment_url_behavior(adapter):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/media/urls/some-link"):
        adapter.persist(doc)
...
def test_persist_enrichment_url_collision_behavior(adapter):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/media/urls/collision"):
...
def test_persist_media_behavior(adapter):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/media/image.png"):
        adapter.persist(doc)
...
def test_persist_media_with_source_path_behavior(adapter, tmp_path):
    ...
    with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/media/moved.png"):
        adapter.persist(doc)
...
def test_persist_pii_skipped(adapter, caplog):
    ...
    doc = Document(
        content=b"secret",
        type=DocumentType.MEDIA,
        metadata={"pii_deleted": True},
        id="pii-doc"
    )

    with caplog.at_level(logging.INFO):
        with patch.object(adapter._url_convention, 'canonical_url', return_value="https://example.com/media/secret.png"):
            adapter.persist(doc)
...
def test_documents_iterator_behavior(adapter):
    ...
    profiles_table = Mock()
    profiles_df = Mock()
    profiles_df.iterrows.return_value = [
        (0, {"content": "---\nname: A1\n---\nBio"})
    ]
    profiles_table.execute.return_value = profiles_df

    def read_table_side_effect(name):
        if name == "posts": return posts_table
        if name == "profiles": return profiles_table
        raise KeyError(name)
...
def test_initialize_filesystem_error(tmp_path):
    ...
    adapter = MkDocsAdapter()
    with pytest.raises(Exception): # DirectoryCreationError maps to OSError or similar
