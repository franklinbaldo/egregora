...
def create_mock_post_for_generator(generator: SiteGenerator, slug: str, title: str, date: str, authors: list[str], tags: list[str], summary: str):
    post_doc = Document(
        type=DocumentType.POST,
        content=f"Content for {title}",
        metadata={"slug": slug, "title": title, "date": date, "authors": authors, "tags": tags, "summary": summary},
    )
...
def create_mock_profile_for_generator(generator: SiteGenerator, author_uuid: str, name: str, bio: str):
    profile_doc = Document(
        ...
        metadata={"uuid": author_uuid, "subject": author_uuid, "name": name, "bio": bio, "slug": f"profile-{author_uuid}"},
    )
...
def test_get_site_stats(site_generator: SiteGenerator):
    create_mock_post_for_generator(site_generator, "post-1", "Post 1", "2025-01-01", ["uuid-1"], ["tag1"], "Summary 1")
    create_mock_post_for_generator(site_generator, "post-2", "Post 2", "2025-01-02", ["uuid-2"], ["tag2"], "Summary 2")
...
def test_get_profiles_data(site_generator: SiteGenerator):
    ...
    create_mock_post_for_generator(site_generator, "post-1", "Post 1", "2025-01-01", ["uuid-1"], ["testing"], "Summary 1")
