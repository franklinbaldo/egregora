...
def test_load_prompt_from_local_file():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            result = generator._load_prompt("test_prompt.md")
...
def test_load_prompt_from_package_resources():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            ...
            with patch("egregora.generator.resources.files") as mock_resources:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            with pytest.raises(ValueError, match="Prompt file .* is empty"):
                generator._load_prompt("empty_prompt.md")
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            with patch("egregora.generator.resources.files") as mock_resources:
                ...
                with pytest.raises(FileNotFoundError, match="Prompt file .* is missing"):
                    generator._load_prompt("nonexistent_prompt.md")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            with patch("egregora.generator.resources.files") as mock_resources:
                ...
                result = generator._load_prompt("priority_test.md")
...
def test_load_prompt_strips_whitespace():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with patch("egregora.generator._PROMPTS_DIR", temp_path):
            result = generator._load_prompt("whitespace_test.md")
