...
def test_internal_entry_to_banner_input_conversion(task_feed: Feed):
    """Verify the internal conversion from Entry to BannerInput is correct."""
    # Arrange
    mock_provider = MockImageGenerationProvider()
    generator = FeedBannerGenerator(provider=mock_provider)
    entry = task_feed.entries[0]

    # Act
    banner_input = generator._entry_to_banner_input(entry)

    # Assert
    assert banner_input.post_title == "Test Title"
    assert banner_input.post_summary == "Test Summary"
    assert banner_input.slug == "test-title"
    assert banner_input.language == "en-US"
