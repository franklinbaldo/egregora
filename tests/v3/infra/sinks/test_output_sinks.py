...
def test_atom_xml_sink_produces_correct_xml(sample_feed: Feed, tmp_path: Path) -> None:
    """Test that sink's XML output matches the standalone XML renderer."""
    output_file = tmp_path / "feed.atom"
    sink = AtomSink(output_path=output_file)

    sink.publish(sample_feed)

    # Output should match the XML string generated by the core function
