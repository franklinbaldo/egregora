...
def test_document_create_factory_defaults():
    content = "# Hello"
    ...
    # Check if id looks like a UUID (default behavior)
    uuid.UUID(doc.id)

def test_document_create_with_semantic_slug():
    slug = "2025-01-01-new-year"
    doc = Document.create(
    ...
    assert doc.id == slug
    # Should NOT be a valid UUIDv4/v5 usually, but it is the ID.

def test_document_create_with_collection():
    doc = Document.create(
        content="Test",
        doc_type=DocumentType.POST,
        title="Test",
