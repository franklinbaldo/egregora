...
These tests run fully offline using deterministic writer/embedding fixtures.
"""
...
class TestWriteCommandWithAPI:
    """Tests for 'egregora write' command with deterministic model stubs."""
    ...
    def test_write_command_end_to_end(
        ...
        """Test complete write command with deterministic model + embeddings."""
        ...
        post_path = docs_dir / "posts" / "deterministic-insights.md"
        assert post_path.exists(), "Expected deterministic post to be written"
        post_content = post_path.read_text(encoding="utf-8")
        assert "Deterministic Post" in post_content
        assert "Synthetic summary generated for CLI validation." in post_content

        profile_path = docs_dir / "profiles" / "ca71a986.md"
        assert profile_path.exists(), "Expected deterministic profile to be written"
        profile_content = profile_path.read_text(encoding="utf-8")
        assert "Profile content generated by CliWriterTestModel" in profile_content
