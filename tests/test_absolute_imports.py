...
def test_relative_imports_are_forbidden() -> None:
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False, dir=Path.cwd()) as tmp:
    ...
    try:
        ...
        assert "TID252" in result.stdout, f"Expected TID252 error in output, got: {result.stdout}"

        # Verify the error message mentions relative imports
        assert any(
            keyword in result.stdout.lower() for keyword in ["relative", "import", "absolute"]
...
def test_absolute_imports_are_allowed() -> None:
    ...
    try:
        ...
        assert "TID252" not in result.stdout, (
