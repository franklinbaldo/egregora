...
EXPECTED_FRANKLIN_OCCURRENCES = 3
EXPECTED_VALID_ZIP_FILES = 3
...
03/10/2025 09:48 - Franklin: Terceira mensagem"""
    ...
    assert franklin_count == EXPECTED_FRANKLIN_OCCURRENCES, (
        f"Franklin should appear {EXPECTED_FRANKLIN_OCCURRENCES} times, found {franklin_count}"
    )
...
def test_edge_cases_handling(temp_dir, whatsapp_real_content):
    ...
    for case in edge_cases:
        try:
            ...
            pytest.fail(f"Edge case failed: {case[:50]}... Error: {error}")
...
def test_zip_listing_functionality(temp_dir):
    ...
    assert len(zip_days) == EXPECTED_VALID_ZIP_FILES
