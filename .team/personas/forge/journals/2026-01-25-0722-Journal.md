...
Action: I implemented a '_clean_metadata' method in 'src/egregora/output_adapters/mkdocs/adapter.py' that removes keys with 'None' values before writing the frontmatter. I applied this sanitizer to all document writing methods ('_write_journal_doc', '_write_profile_doc', '_write_enrichment_doc', '_write_annotation_doc'). I also fixed a bug in '_write_profile_doc' where the 'posts' list was being omitted from the frontmatter because 'yaml.dump' was called too early. I verified the fix with a new unit test 'tests/unit/output_adapters/mkdocs/test_adapter_metadata.py' and by confirming the demo site builds successfully (even without content).
