...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("/tmp/egregora_test_run.log"),
...
def main():
    ...
    logger.info(f"Window Configuration: 1 day (step_size=1, step_unit='days')")
    logger.info("=" * 80)
    ...
    try:
        ...
        if posts_dir.exists():
            ...
            for post_file in post_files[:10]:  # Show first 10
                logger.info(f"  - {post_file.name}")
            if len(post_files) > 10:
                logger.info(f"  ... and {len(post_files) - 10} more")
        ...
        logger.error(f"Issues logged to: {issues_file}")
        return 1
        ...
        with summary_file.open("w") as f:
            ...
            f.write(f"Window: 1 day (step_size=1, step_unit='days')\n")
            f.write(f"Enrichment: Enabled\n")
