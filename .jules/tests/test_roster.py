...
from jules.cli.roster import app, get_personas_dir
...
class TestGetPersonasDir:
    ...
    def test_finds_jules_personas_dir(self, tmp_path, monkeypatch):
        """Should find .jules/personas when it exists."""
    ...
    def test_finds_personas_dir(self, tmp_path, monkeypatch):
        """Should find personas/ when .jules/personas doesn't exist."""
...
class TestRosterIntegration:
    ...
    @pytest.mark.skipif(
        not Path(".jules/personas").exists(),
