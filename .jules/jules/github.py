...
def get_pr_details_via_gh(pr_number: int, repo_path: str = ".") -> Dict[str, Any]:
    ...
    try:
        pr_data = run_gh_command([
            ...
            "--json", "title,body,headRefName,baseRefName,isDraft,mergeable,statusCheckRollup,files,comments,reviews,latestReviews,commits,author"
        ], cwd=repo_path)
    except Exception:
        raise Exception(f"Failed to view PR {pr_number}")
...
def _extract_session_id(branch: str, body: str) -> Optional[str]:
