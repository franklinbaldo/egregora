...
cat .jules/PARALLEL_PERSONAS_PROMPT.md
...
# Read the prompt
with open('.jules/PARALLEL_PERSONAS_PROMPT.md') as f:
    prompt = f.read()
...
2. **Check status** - See `.jules/jules/client.py` for API usage
...
```bash
# Test mail backend
pytest tests/unit/jules/mail/ -v
...
python .jules/jules/scheduler_v2.py --dry-run --mode cycle
...
### Week 1: Foundation
- ✅ Mail backend (`jules/mail.py`)
- ✅ Mail CLI (`jules/mail_cli.py`)
- ✅ Comprehensive tests
...
- Read `.jules/README.md` for personas overview
