 ...
 */
function createReviewPrompt(repomixContent, patchContent) {
  return `You are a senior software engineer conducting a thorough, candid code review. You will receive:
1. A complete repository context (generated by Repomix)
2. A Git patch showing the changes in this pull request

Your task is to provide a comprehensive, actionable code review in Markdown format.
...
Provide a **candid, professional review** covering these areas:
...
- **Be specific** - cite exact locations (file:line) when possible
- **Be actionable** - provide concrete suggestions, not just problems
- **Focus on substance** - avoid unnecessary praise or flattery
- **Prioritize** - distinguish critical issues from minor nitpicks
...
Structure your review using the sections above as Markdown headings (##).`;
}

/**
 * Send review request to Gemini
 */
async function getGeminiReview(repomixContent, patchContent) {
  ...
  const prompt = createReviewPrompt(repomixContent, patchContent);
...
async function main() {
  ...
  const review = await getGeminiReview(repomixContent, patchContent);
