...
def get_suggestion(filename: str) -> str:
    ...
    if any(x in name_lower for x in [
        "log", "diff", "report", "output", "zip", "json", "txt", "xml", "coverage", "dump"
    ...
    if name_lower.endswith(".md") or any(x in name_lower for x in ["plan", "todo", "idea", "draft", "analysis", "review"]):
        return "notes/"

    # Scripts
    if name_lower.endswith(".py") or name_lower.endswith(".sh"):
        return "scripts/"
