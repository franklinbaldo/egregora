...
PRODUCTION_READY_SCORE = 85
NEEDS_REVIEW_SCORE = 70
MIN_FIO_COUNT = 4
MAX_FIO_COUNT = 10
MIN_FIO_COUNT_RELAXED = 3
MAX_FIO_COUNT_RELAXED = 12
MIN_SUBSTANTIVE_SENTENCE_LENGTH = 20
HIGH_ATTRIBUTION_RATE = 0.95
MEDIUM_ATTRIBUTION_RATE = 0.85
LOW_ATTRIBUTION_RATE = 0.70
HIGH_CONTEXT_DENSITY = 0.02
MEDIUM_CONTEXT_DENSITY = 0.015
...
class QualityResult:
    ...
    def status(self) -> str:
        ...
        elif self.overall_score >= PRODUCTION_READY_SCORE:
            return "PRODUCTION READY"
        elif self.overall_score >= NEEDS_REVIEW_SCORE:
            return "NEEDS REVIEW"
        else:
...
class PostQualityValidator:
    ...
    def _extract_yaml_and_content(self, content: str) -> tuple[dict | None, str]:
...
", 4)
    ...
    def _validate_structure(self, yaml_data: dict | None, content: str) -> int:
        ...
        if MIN_FIO_COUNT <= fio_count <= MAX_FIO_COUNT:
            score += 10
        elif MIN_FIO_COUNT_RELAXED <= fio_count <= MAX_FIO_COUNT_RELAXED:
            score += 7
    ...
    def _validate_content_quality(self, content: str) -> int:
        ...
        plural_indicators = len(
            re.findall(r"\bnós\b|\bnosso\b|\bnossa\b|\bconosco\b", content, re.IGNORECASE)
        )
        singular_indicators = len(
            re.findall(r"\beu\b|\bmeu\b|\bminha\b|\bcomigo\b", content, re.IGNORECASE)
        )
        ...
        substantive_sentences = [
            s for s in sentences if len(s.strip()) > MIN_SUBSTANTIVE_SENTENCE_LENGTH and not s.strip().startswith("#")
        ]
        ...
        if substantive_sentences:
            ...
            if attribution_rate >= HIGH_ATTRIBUTION_RATE:
                score += 10
            elif attribution_rate >= MEDIUM_ATTRIBUTION_RATE:
                score += 8
                self.warnings.append(f"Attribution coverage: {attribution_rate:.1%}")
            elif attribution_rate >= LOW_ATTRIBUTION_RATE:
                score += 5
        ...
        context_indicators = len(
            re.findall(
                r"\bporque\b|\bquando\b|\bcomo\b|\bentão\b|\bassim\b|\bpois\b",
                content,
                re.IGNORECASE,
            )
        )
        ...
        if content_length > 0:
            ...
            if context_density >= HIGH_CONTEXT_DENSITY:
                score += 7
            elif context_density >= MEDIUM_CONTEXT_DENSITY:
                score += 5
    ...
    def _validate_technical_aspects(self, yaml_data: dict | None, content: str) -> int:
        ...
        if content.count("#") >= MIN_MARKDOWN_HEADERS:  # Has headers
            score += 5
        else:
            score += 2
...
"""
        ...
        if result.overall_score >= PRODUCTION_READY_SCORE:
            report += "✅ RECOMMENDATION: Ready for production\n"
        elif result.overall_score >= NEEDS_REVIEW_SCORE:
